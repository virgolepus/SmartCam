...
void on_mouse(int event,int x,int y,int flag, void *param)
{
    ...
    if(event==CV_EVENT_MOUSEMOVE && startDraw)
	{
		...
		cvRectangle(image2,cvPoint(roi_x0,roi_y0),cvPoint(x,y),cvScalar(255,0,255),1);
			...
			do
			{
				...
				//printf("   %d. rect x=%d\ty=%d\twidth=%d\theight=%d\n",numOfRec,roi_x1,roi_y1,roi_x0-roi_x1,roi_y0-roi_y1);
						...
						if(roi_x0<roi_x1 && roi_y0<roi_y1)
						{
							...
							strPostfix+=" "+Float2String(roi_x0)+" "+Float2String(roi_y0)+" "+Float2String(roi_x1)+" "+Float2String(roi_y1)+" "+Float2String(width)+" "+Float2String(height);
						}
						if(roi_x0>roi_x1 && roi_y0>roi_y1)
						{
							printf("   %d. rect x=%d\ty=%d\twidth=%d\theight=%d\n",numOfRec,roi_x1,roi_y1,roi_x0-roi_x1,roi_y0-roi_y1);
							// append rectangle coord to previous line content
							strPostfix+=" "+Float2String(roi_x1)+" "+Float2String(roi_y1)+" "+Float2String(roi_x0)+" "+Float2String(roi_y0)+" "+Float2String(width)+" "+Float2String(height);
						}
						break;
				}
			}
